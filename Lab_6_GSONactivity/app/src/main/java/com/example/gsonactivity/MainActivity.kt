...
interface CellClickListener {
    fun onCellClickListener(text: String, context: Context) {
        val questionIntent = Intent(context, PicViewer::class.java)
        questionIntent.putExtra("pickLink", text)
        startActivityForResult(context as Activity, questionIntent, 1, Bundle.EMPTY)
    }
}
...
class MainActivity : AppCompatActivity(), CellClickListener {
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == RESULT_OK) {
            when (requestCode) {
                1 -> {
                    val res = data?.getStringExtra("answer")
                    val snackbar = Snackbar.make(findViewById(R.id.myLayout), "Картинка добавлена в избранное", 1)
                        val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse(res))
